...
## تحسينات ديسمبر 2025 (الأحدث)

### 1. إصلاح مؤشر قوة الإشارة (Signal Bars)
- إضافة `SignalBarsToOpacityConverter` لتحويل قيمة الإشارة إلى شفافية
- كل شريط يظهر فقط عندما تصل قوة الإشارة لعتبته:
  - الشريط 1: SignalBars >= 1
  - الشريط 2: SignalBars >= 2
  - الشريط 3: SignalBars >= 3
  - الشريط 4: SignalBars >= 4
  - الشريط 5: SignalBars >= 5
- الشفافية: 1.0 للشريط المضاء، 0.25 للشريط الخامد

### 2. إصلاح قراءة أوامر AT
- تم إزالة `+CUSD` من شروط توقف القراءة في `SendATCommandAsync`
- الآن يتوقف فقط عند `OK` أو `ERROR`
- هذا يمنع قطع الردود من الإشعارات غير المتوقعة

### 3. تحسين جلب رقم الهاتف
- إضافة أنماط regex متعددة لتغطية صيغ CNUM المختلفة:
  ```
  +CNUM: "Name", "+20123456789"
  +CNUM: , "+20123456789"
  +CNUM: "+20123456789"
  ```
- إرجاع "غير معروف" بشكل موحد عند الفشل (بدلاً من رسائل الخطأ)
- التحقق من طول الرقم (10-15 رقم على الأقل)
